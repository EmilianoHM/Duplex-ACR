    ...
    try{
        ...
        System.out.println("\nElegiste "+ni.getDisplayName());
          ...
          SocketAddress dir;
            ...
            s.joinGroup(dir, ni);
            //s.joinGroup(gpo);
            String msj="Mensaje desde el servidor 1";
            String msjunicast ="anuncio via unicast";
            byte[] b = msj.getBytes();
            byte[] b1 = msjunicast.getBytes();
            InetAddress dstunicast = InetAddress.getByName("127.0.0.1");
            ...
            for(;;){
              ...
              s.send(p);
              DatagramPacket p1 = new DatagramPacket(b1,b1.length,dstunicast,5555);
              s.send(p1);
