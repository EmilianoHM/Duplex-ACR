...
class wget2 implements Runnable {
    ...
    static void download_write(String url, String urlserver, String path, int nivel) {
        try {
            // Descargar el contenido del index utilizando HttpURLConnection
            URL urlObject = new URL(url);
            HttpURLConnection connection = (HttpURLConnection) urlObject.openConnection();
            connection.setRequestMethod("GET"); // Configurar el método HTTP como GET
            connection.setRequestProperty("User-Agent", "Mozilla/5.0"); // Simula un agente de usuario de navegador
    ...
    static void getData_Download(String s, String[] parts) {
        ...
        System.out.println("La dirección anterior es: " + previous);
        System.out.println("La dirección del servidor o de origen es: " + servername);
        System.out.println("--  --  --  --  --  --  --  --  --  --  --  --  --");
    ...
    public void run() {
        ...
        String dirname = "";
        boolean resource;
        System.out.println ("NOTA: para descargar de modo recursivo debemos ingresar: '-r' seguido del link ");
...
public class WGET {
    ...
    public static void main(String[] args) {
        int tamanioPool;
        Scanner input = new Scanner(System.in);
       System.out.println("Introduce el tamaño del pool de hilos: ");
        tamanioPool = input.nextInt();
        Runnable g = new wget2();
        ExecutorService pool = Executors.newFixedThreadPool(tamanioPool);
        pool.execute(g);
        pool.shutdown();
    }
}

//https://i.ibb.co/KL4k2BT/DALL-E-2023-12-13-11-22-05-An-inviting-and-cheerful-welcome-image-for-customers-using-the-Coffee-Sys.png
